...
// 半成品对象。这样 ?renderer=webgpu 会响亮失败，绝不悄悄退回 canvas/dom 掩盖问题
// —— 与 xterm.js「WebGL 失败退回 DOM」的做法相反。

/**
 * 构造 WebGPU renderer（v2）。当前是接口桩：立即 throw，未实现任何绘制逻辑。
 *
 * @param {HTMLCanvasElement} canvas 目标画布（v2 在其上请求 GPU device）
 * @param {number} cols 列数（v2 用于字形单元几何）
 * @param {number} rows 行数
 */
export function createWebGPURenderer(canvas, cols, rows) {
  throw new Error('WebGPURenderer 是 v2（纹理图集 + instanced quad），未实现');
}
